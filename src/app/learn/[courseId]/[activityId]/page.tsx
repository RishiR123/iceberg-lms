...
export default async function ActivityPage({
  ...
  return (
    <div className="flex-1 flex flex-col w-full max-w-5xl mx-auto px-6 md:px-8 py-10 bg-[#F5EFFF]/10 min-h-full font-sans selection:bg-[#E9D5FF] selection:text-[#0B012C]">
      
      {/* Upper Navigation / Metadata Header */}
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-5 border-b border-[#E2D5F8]/40 pb-6">
        <div className="space-y-1.5">
          <span className="inline-flex items-center text-[10px] font-bold text-[#0B012C] uppercase tracking-wider bg-[#E9D5FF] px-2.5 py-1 rounded-full border border-[#E2D5F8]/40">
            {typeConfig.icon} {typeConfig.label} • Activity {currentIndex + 1} of {flatActivities.length}
          </span>
          <h1 className="text-2xl md:text-3.5xl font-black tracking-tight text-[#0B012C]">
            {activity.title}
          </h1>
          {activity.duration && (
      ...
      <div className="flex-1 space-y-8">
        ...
        {parsedResources.length > 0 && (
          <div className="space-y-4 mt-10 border-t border-[#E2D5F8]/40 pt-8">
            <h4 className="text-xs font-black uppercase tracking-wider text-[#645A95] flex items-center gap-1.5">
              <Lightbulb className="w-4 h-4 text-[#0B012C]" /> Downloadable Resource Guides
            </h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {parsedResources.map((res, rIdx) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="p-5 border border-[#E2D5F8]/50 rounded-2xl flex items-center bg-white hover:bg-[#F5EFFF]/30 hover:border-[#E2D5F8] transition-all cursor-pointer group shadow-sm hover:-translate-y-0.5"
                >
                  <div className="w-10 h-10 rounded-xl bg-[#E9D5FF] text-[#0B012C] flex items-center justify-center mr-4 group-hover:scale-105 transition-transform">
                    <FileText className="w-4 h-4" />
                  </div>
                  <div className="flex flex-col flex-1 min-w-0 text-left">
                    <span className="font-extrabold text-xs md:text-sm text-[#0B012C] truncate group-hover:underline flex items-center gap-1">
                      {res.title} <ExternalLink className="w-3 h-3 text-[#645A95]" />
                    </span>
                    <span className="text-[10px] text-slate-400 mt-0.5 font-medium">External reference resource</span>
                  </div>
                  <Download className="w-4 h-4 text-[#645A95] flex-shrink-0 ml-2" />
                </a>
              ))}
            </div>
          </div>
      ...
      <div className="flex items-center justify-between pt-6 border-t border-[#E2D5F8]/40 mt-16 bg-white/50 rounded-2xl p-5 shadow-sm">
        {prevActivity ? (
          <Link
            href={`/learn/${courseId}/${prevActivity.id}`}
            className="flex flex-col items-start px-4 py-3 rounded-xl border border-[#E2D5F8]/50 bg-white hover:bg-[#F5EFFF]/20 transition-all group max-w-[45%] min-w-[120px] text-left"
          >
            ...
            <span className="font-extrabold text-xs text-[#0B012C] truncate w-full">{prevActivity.title}</span>
          </Link>
        ) : (
          <div />
        )}
        ...
        {nextActivity ? (
          <Link
            ...
            className="flex flex-col items-end px-5 py-3 rounded-xl bg-[#0B012C] hover:bg-[#0B012C]/90 text-white transition-all group text-right max-w-[45%] min-w-[120px] shadow-sm hover:shadow-md"
          >
            <span className="text-[9px] font-bold text-[#E9D5FF] uppercase tracking-wider mb-1 flex items-center">
          ...
          <div className="flex flex-col items-end px-5 py-3 rounded-xl bg-[#D9F99D] text-[#0B012C] text-right max-w-[45%] min-w-[120px] shadow-sm border border-[#0B012C]/15">
             <span className="text-[9px] font-extrabold uppercase tracking-wider mb-1">🎉 Course Complete</span>
             <span className="font-black text-xs text-[#0B012C]">You finished all units!</span>
          </div>
        )}
      </div>
    </div>
