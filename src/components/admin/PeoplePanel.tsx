    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div className="space-y-1">
          <h2 className="text-sm font-black text-[#0F172A]">People</h2>
      ...
      {reset && (
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 space-y-3">
          ...
          <div className="bg-white rounded-xl border border-amber-200 p-3 font-mono text-[11px] text-[#0F172A]">
            {reset.password}
          </div>
          <button
            onClick={async () => {
      ...
      {users.length > 5 && (
        <div className="relative max-w-xs">
          ...
          <input
            ...
            className="w-full text-xs border border-[#E2E8F0]/60 pl-9 pr-3 py-2 rounded-xl bg-white focus:outline-none focus:border-[#4F46E5]/60 font-semibold text-[#0F172A]"
          />
        </div>
      )}
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[720px] text-left border-collapse">
          ...
          <tbody>
            {visible.length === 0 ? (
              <tr>
                <td colSpan={6} className="py-8 text-center text-xs text-[#64748B] font-semibold">
              ...
              visible.map((u) => {
                ...
                return (
                  <Fragment key={u.id}>
                  <tr className="border-b border-[#E2E8F0]/25 hover:bg-[#F8FAFC]/25 transition-colors">
                    <td className="py-3 px-2 pl-0">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 flex-shrink-0 rounded-xl bg-[#0F172A] text-white flex items-center justify-center font-black text-[11px]">
                          {u.name.slice(0, 1).toUpperCase()}
                        </div>
                        <div className="min-w-0">
                          <div className="text-xs font-bold text-[#0F172A] truncate">
                            {u.name}
                            {isSelf && <span className="text-[9px] text-[#64748B] font-semibold ml-1.5">(you)</span>}
                          </div>
                          <div className="text-[10px] text-[#64748B] font-semibold truncate">{u.email}</div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-3 px-2">
                      <button
                        ...
                        className="inline-flex items-center gap-1 text-xs font-semibold text-[#0F172A] hover:text-[#4F46E5] transition-colors cursor-pointer"
                    ...
                    <td className="py-3 px-2 text-xs font-semibold text-[#0F172A]">{u.activitiesCompleted}</td>
                    <td className="py-3 px-2 text-[10px] font-semibold text-[#64748B] whitespace-nowrap">
                      {new Date(u.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
                    </td>

                    <td className="py-3 px-2 pr-0">
                      <div className="flex items-center justify-end gap-3">
                        {busy && <Loader2 className="w-3 h-3 animate-spin text-[#64748B]" />}

                        <button
                          onClick={() => run(u.id, () => setUserRoleAction(u.id, u.role === "ADMIN" ? "STUDENT" : "ADMIN"))}
                          disabled={busy}
                          className="text-[10px] font-bold text-[#4F46E5] hover:underline disabled:opacity-40 cursor-pointer whitespace-nowrap"
                        >
                          ...
                          (confirmId === u.id ? (
                            <span className="flex items-center gap-1.5 whitespace-nowrap">
                              ...
                              <button
                                ...
                                className="text-[10px] font-semibold text-[#64748B] hover:underline cursor-pointer"
                              >
                                Cancel
                              </button>
                            </span>
                  ...
                  {expandedId === u.id && (
                    <tr className="border-b border-[#E2E8F0]/25 bg-[#F8FAFC]/20">
